...
public class TopMobKillsAction {

    // Метод для получения имени игрока по индексу
    public static String getTopMobKillsName(List<Map.Entry<String, Integer>> sortedStats, String params) {
        try {
    ...
    public static String getTopMobKillsValue(List<Map.Entry<String, Integer>> sortedStats, String params) {
        try {
            int index = Integer.parseInt(params.replaceAll("\\D+", "")) - 1;
            if (index >= 0 && index < sortedStats.size()) {
